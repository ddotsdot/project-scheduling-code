...
public class OOE_h_Precedences_DIS<CONST
    extends IConstants_E & IConstants_P_bar & IConstants_M_i>
    extends AbstractOrtoolsConstraintManager<CONST> {

  public OOE_h_Precedences_DIS() {
    super("i", "j", "m", "e");
  }
  ...
  protected Collection<ConstraintKey> createKeys(CONST constants) {
    ...
    for (Precedence precedence : constants.get_P_bar()) {
      for (int m : constants.get_M_i(precedence.get_i())) {
        for (int e : constants.get_E()) {
          if (e != constants.get_card_A()) {
            indexes.add(new ConstraintKey(precedence.get_i(), precedence.get_j(), m, e));
          }
        }
      }
    }
  ...
  protected void configureConstraint(
    ...
    for (int mPrime : constants.get_M_i(j)) {
      for (int ePrime = 0; ePrime <= e; ePrime++) {
        constraint.setCoefficient(
            variables.getVar(DecisionVars_Ooe.z, j, mPrime, ePrime),
            1.);
      }
    }
  }
